...
$db   = get_db();
$site = $db->query('SELECT wp_url FROM sites LIMIT 1')->fetch();
...
<title>Connect Claude to <?php echo htmlspecialchars($site_domain); ?></title>
<style>
...
.card{background:#fff;border-radius:14px;box-shadow:0 4px 28px rgba(0,0,0,.10);padding:2.2rem 2.4rem;max-width:400px;width:100%;text-align:center}
...
p{font-size:.825rem;color:#646970;line-height:1.65;margin-bottom:1.6rem}
...
<div class="card">
    ...
    <div class="domain"><?php echo htmlspecialchars($site_domain); ?></div>
    <p>Claude will be able to read and manage content on your WordPress site. You can disconnect at any time from the AISA Connector page in wp-admin.</p>
    <form method="POST">
