...
4. **Hosting.** ~~Current host vs Vercel (connector already available in owner's Claude setup). Vercel pairs naturally with a static/Next build.~~ **RESOLVED 21 Sep 2026 (owner): Vercel is ruled out.** Production goes the way the preview pipeline already goes, a static export pushed to the current host over FTPS by `.github/workflows/deploy-preview.yml`. No Vercel-dependent feature is available: no per-branch preview URLs, no middleware, no ISR, no edge functions. Note that §11's "enforced at edge (Vercel)" line for the redirect map is superseded by the same ruling, and redirects ship as `public/.htaccess` generated by `scripts/gen-*-redirects.mjs`.
...
## 27. AMENDMENT 6 — HOST AND STORAGE CLOSED (owner, 21 September 2026)

Supersedes §5.4, and the "enforced at edge (Vercel)" line in §11. Where
§14, §17 or §25 imply a hosting or storage choice, this section wins.

### Vercel is ruled out

Production goes the way the preview pipeline already goes: a static export
built by `.github/workflows/deploy-preview.yml` and pushed over FTPS, as
valenciamove.com is hosted. The question is closed rather than open, so do
not propose Vercel again.

What follows from it, because each is a feature somebody will otherwise
reach for on the assumption it exists:

- **No per-branch preview URLs.** A reviewer has no clickable preview of a
  PR. Say so in a PR body rather than implying one.
- **No middleware, no ISR, no on-demand revalidation, no edge functions.**
  The build is `output: "export"`; anything needing a server at request
  time is out of scope by construction.
- **Redirects ship as `public/.htaccess`**, generated by the
  `scripts/gen-*-redirects.mjs` family. Never `next.config` `redirects()`,
  which does not run under a static export, and never "at the edge".
- **Cache and CDN behaviour belongs to the current host**, and needs
  verifying against it rather than assumed from Vercel's documentation.

### The database stays out, with one exception

§25 removed Payload and with it the database. The owner reaffirmed that on
21 September 2026 while reviewing a proposal to adopt Supabase, and left
exactly one opening: **heavy video**.

If the site ever carries video too large to live in the repo and ship
through a static export, object storage is in scope for those files. For
nothing else.

The distinction is the whole of it, and it is where the closed decision
would come back if anyone is loose about it. Putting an MP4 in a bucket is
storing a media file. Moving text, page structure or metadata into a
queryable store is the decision that was closed, whatever product name it
arrives under and however reasonable the feature asking for it sounds.

`/public/images/` and the static export remain the media path for
everything that is not heavy video. `cto` holds this line.
