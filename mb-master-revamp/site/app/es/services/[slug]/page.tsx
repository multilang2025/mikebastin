...
// The 10 ES service pages qualifying for a live page per
// redirects/content-map.json (`type: "service"`, `action: "migrate"`,
// `destination: "mdx"`, an `es` entry -- g115 has no `es` entry, so it is
// FR-only). At /es/services/<slug>/, matching each file's own sourceUrl
// frontmatter and the pre-existing WordPress URL structure.
...
export default async function SpanishServicePage({
  ...
  const url = `${SITE_URL}/es/services/${service.slug}/`;
