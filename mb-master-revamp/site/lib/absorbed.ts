/**
 * What each service page absorbed, written as prose.
 *
 ...
 * reader, who learns nothing from it and fairly reads the section as
 * unfinished. A tidier list of names and one-liners was no better: a
 * reader skims a list and takes nothing from it, and the whole reason for
 * keeping the section is that several merged pages' worth of subject
 * matter should still be readable on the page that absorbed them.
 *
 * So it is prose, per service, written from those pages' own harvested
 * copy in site/content/en/ and stripped of the legacy marketing voice
 * ("Revolutionise Your Global Reach", "We Dream Big"). The specialisms
 * are named inside the sentences, which keeps the query coverage the
 * merge was supposed to preserve without reading like a directory.
 *
...
export const ABSORBED_PROSE: Record<string, string[]> = {
  "multilingual-seo": [
    "Most of what used to sit on four separate pages happens before a word gets written. Which markets to enter, in what order, and where the budget goes, decided against what a market is worth rather than which language is easiest to buy.",
  ...
  "website-localisation": [
    "Six pages used to split this up, and in practice it is one job with six places to get it wrong. Content first: pages that read as though written for the market, not translated into it.",
  ...
  "translation-services": [
    "Eight pages used to divide this by document type, which is a fair way to think about it, because the risk changes completely from one to the next.",
  ...
  "app-and-software-localisation": [
    "Three pages used to cover this, and the order matters: internationalisation is what you do before launch, localisation is what you do after.",
  ...
  "technical-seo": [
    "Five pages used to carve this up. What they had in common is that all of it sits inside your control, which is not true of much else in search.",
